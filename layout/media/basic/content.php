<div class="pure-g">
	
	<div class="upg_basic_title"><?php echo $post_title; ?></div>
	
	 <div class="pure-u-1-1" style="text-align:center;"><?php echo upg_position1(); ?></div>
	
	<div class="pure-u-1-1" > 

	<div class="margin-box">
	<center>
	<?php 
		if($post_status=="draft")
		echo '<div class="upg_tooltip"><i class="fas fa-eye-slash"></i><span class="upg_tooltiptext">'. __("Under review","wp-upg").'</span></div>'; 
	?>
	<?php do_action( "upg_layout_up"); ?>
	<?php
					 if(upg_isVideo($post))
					 {
						 $attr = array(
						'src'      => esc_url( upg_isVideo($post) ),
						'width'    => 560,
						'height'   => 315
						
						);
	
						echo wp_video_shortcode( $attr );
					 }
					else
					{
							 
						?>
					 <div class="upg_image-frame"><img src="<?php echo $image; ?>"></div>
					  <?php
					}
						 ?>
	
	</center>
	</div>


	</div>
	
	<div class="pure-u-1 pure-u-md-3-5">
		<div class="margin-box">
	
	
			<div class="upg-desp"> 	<?php echo $text; ?> </div>
	
		</div>
	</div>
	 <div class="pure-u-1 pure-u-md-2-5"> 
		<div class="margin-box">
	
	<?php
		//Display 5 custom fields loop
		for ($x = 1; $x <= 5; $x++) 
		{
			if($options['upg_custom_field_'.$x.'_show_front']=='on')
			{
				
			?>
			<div class="pure-u-1"><?php echo upg_get_filed_label('upg_custom_field_'.$x); ?> : <?php echo upg_get_value('upg_custom_field_'.$x); ?></div>
			
			<?php
				
			}
		}
		
		?>
		</div>

	</div>
	
	<div class="pure-u-1 pure-u-md-4-5"><?php upg_list_tags($post); ?> <br><?php do_action( "upg_layout_down"); ?> <?php echo upg_position2(); ?>
	</div>
	
	<div class="pure-u-1 pure-u-md-1-5"> 
		<?php echo upg_author($author,false); ?>
	</div>
	
</div>	